...
MA_PRESETS = [
    ...
    "Уровень 0–10 м",
    "Уровень 0–100 %",
    "Давление 0–10 бар (инверсный)",
    "Уровень 0–100 % (инверсный)",
]
...
MA_PRESET_DATA = {
    ...
    "Давление 0–10 бар (инверсный)": (20, 4, 0, 10, "бар"),
    "Уровень 0–100 % (инверсный)": (20, 4, 0, 100, "%"),
}
...
class MaScreen(Screen):
    ...
    def on_calc(self, *_):
        try:
            ...
            # Инверсный диапазон (например, 20–4 мА) допустим
            if ma_max == ma_min:
                raise ValueError("Границы сигнала не должны совпадать")
            if val_max == val_min:
                raise ValueError("Границы величины не должны совпадать")
            unit = self.s_unit.text
            if self._ma2val:
                pct = (inp - ma_min) / (ma_max - ma_min) * 100
                val = val_min + (inp - ma_min) / (ma_max - ma_min) * (val_max - val_min)
                out_of = ""
                if not (min(ma_min, ma_max) <= inp <= max(ma_min, ma_max)):
                    out_of = " [!] ВНЕ ДИАПАЗОНА"
                self.l_res.text = f"{val:.3f} {unit}  ({pct + 0.0:.1f}%){out_of}"
            else:
                pct = (inp - val_min) / (val_max - val_min) * 100
                ma = ma_min + (inp - val_min) / (val_max - val_min) * (ma_max - ma_min)
                out_of = ""
                if not (min(val_min, val_max) <= inp <= max(val_min, val_max)):
                    out_of = " [!] ВНЕ ДИАПАЗОНА"
                self.l_res.text = f"{ma:.3f} мА  ({pct + 0.0:.1f}%){out_of}"
        except InputError as e:
            self.l_res.text = f"Ошибка ввода: {e}"
            popup("Ошибка ввода", str(e), "error")
        except ValueError as e:
...
class AboutScreen(Screen):
    def __init__(self, **kw):
        ...
        content.add_widget(make_label("Расчётный модуль КИПиА", size=22, bold=True,
                                      color=PRIMARY))
        self.l_ver = make_label("Версия 1.3", size=14, color=MUTED)
        content.add_widget(self.l_ver)

        self.l_about_1 = make_result_area(content)
        content.add_widget(self.l_about_1)
        self.l_about_1.text = (
            "Программа предназначена для специалистов КИПиА, метрологов "
            "и инженеров.\n"
            "Помогает быстро и точно выполнять расчёты при поверке, "
        ...
        self.l_about_2.text = (
            ...
            "в том числе инверсного (пресеты приборов)\n"
            "• Расход по квадратичной зависимости: шкала и сигнал\n"
            "• Диагностика датчиков и мини-протокол поверки\n"
            "• Термометры сопротивления: НСХ, пересчёт R и t, таблицы поверки\n"
            "• Термопары: НСХ, пересчёт ЭДС и t, таблицы поверки\n"
        ...
        heading(content, "Что нового в версии 1.3")
        self.l_about_new = make_result_area(content)
        content.add_widget(self.l_about_new)
        self.l_about_new.text = (
            "• Вкладка «4–20 мА / Величина»: поддержка инверсного сигнала "
            "(например, 20–4 мА)\n"
            "• Новые пресеты: давление 0–10 бар и уровень 0–100 % "
            "с инверсным сигналом\n"
            "• Импульсные линии: время быстрого остывания теперь зависит "
            "от объёма линии")
